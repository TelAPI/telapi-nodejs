...
module.exports = function(account_sid, auth_token) {
    ...
    /**
     * TelAPI account sid that is unique to account itself. AccountSid can be
    ...
    /**
     * TelAPI auth token that is unique to account itself. AuthToken can be
     * found at: http://www.telapi.com/dashboard
     *
    ...
    /**
     * TelAPI base URI. This is used for internal purposes while building URL
     *
    ...
    /**
     * TelAPI contant type. At this moment Node.JS helper supports only JSON so please do not
     * change variable to anything else.
     *
    ...
    this.request_postdata     = {};
     ...
     /**
     * TelAPI INTERNAL: Used as schema validation helper
     *
    ...
    /**
     * Method used to check whenever resource exists or not
     *
     ...
     * one of required resource parameters are missing, error will be thrown
     *
     * @param {Object}   post_data: (Required) Data that will be sent with request
     */
    ...
    /**
     * Internal method used to send requests against TelAPI API
     *
     * @param {Function} success:    (Optional) Success callback
     * @param {Function} error:      (Optional) Error callback
     */
        ...
        var request = https.request(request_options, function(response) {
            ...
            response.on('end', function() {
                ...
                if ( response.statusCode != 200 ) {
                    if(body.status != 200) {
                        throw new Error("TelAPI Error: '" + body.message + "'. More info URL: '" + body.more_info + "'");
                    }
                }
