...
var TelAPI = module.exports.TelAPI = function(account_sid, auth_token) {
    ...
    this.get         = function(resource, parameters, success, error) {
        util.log("[INFO] GET RESOURCE DETAILS: " + resource);
        util.log("[INFO] GET BUILT URL IS: " + this._buildUrl(resource, parameters));
        this._run(success, error);
    };

    this.create      = function(resource, post_data, success, error) {
        util.log("[INFO] CREATE RESOURCE DETAILS: " + resource);
        util.log("[INFO] CREATE BUILT URL IS: " + this._buildUrl(resource, {}));
