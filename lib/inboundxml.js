 ...
 * @param {Map} attrs: (Optional) Attributes for this XML element.
 *
 * var s = new InboundXML.Say('Tell me your favourite number');
 * (new InboundXML.Gather(p, {maxDigits: 2})).toString() returns:
 *
 * <Gather maxDigits="2">
 *     <Say>
 ...
 * event emitter.
 *
 * @event 'gathered': Fired when InboundXML has queried the gather URI
 * generated by AutoUri.
 * Example:
 *     var p = new Gather(); // Empty Gather, which is valid
 *     p.on('gathered', function(reqParams, res) {
 *         // Here, reqParams is the posted data from InboundXML's request
 *         // and Response is a InboundXML.Response object.
 *         // With 'Gathers', InboundXML's request includes a Digits param
 ...
 *
 * @event 'recorded': Fired when InboundXML has requested the auto-generated action
 * uri for this element.
 *
 * Example:
 *    var r = new InboundXML.Record({timeout: 10});
 *    r.on('recorded', function(reqParam, res) {
 *        // Just like with gather, reqParams are the InboundXML request's params
 *        // and res is a InboundXML.Response object.
 *        // InboundXML will send a RecordingUrl parameter with the URL of the recording.
